...
use crate::{
    ...
    models::{ChangeSymptom, MaybeString, NewSymptom, Symptom, UserId},
};
use classes::classes;
    ...
    match op {
        ...
        Operation::Update { symptom } => {
            let changes = ChangeSymptom {
                user_id: None,
                time: Some(time),
                appetite_loss: Some(appetite_loss),
                fever: Some(fever),
                cough: Some(cough),
                sore_throat: Some(sore_throat),
                runny_nose: Some(runny_nose),
                sneezing: Some(sneezing),
                heart_burn: Some(heart_burn),
                abdominal_pain: Some(abdominal_pain),
                abdominal_pain_location: Some(abdominal_pain_location),
                diarrhea: Some(diarrhea),
                constipation: Some(constipation),
                lower_back_pain: Some(lower_back_pain),
                upper_back_pain: Some(upper_back_pain),
                neck_pain: Some(neck_pain),
                joint_pain: Some(joint_pain),
                headache: Some(headache),
                nausea: Some(nausea),
                dizziness: Some(dizziness),
                stomach_ache: Some(stomach_ache),
                chest_pain: Some(chest_pain),
                shortness_of_breath: Some(shortness_of_breath),
                fatigue: Some(fatigue),
                anxiety: Some(anxiety),
