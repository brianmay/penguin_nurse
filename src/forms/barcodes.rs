...
fn BarcodeInput(on_set: Callback<String>) -> Element {
    let stop_flag_id = use_memo(|| {
        ...
        format!("__barcode_stop_{}", COUNTER.fetch_add(1, Ordering::Relaxed))
    });

    // On unmount: set the stop flag and stop the camera track directly via
    // the stream reference stored on window.
                ...
                while (!window['{flag}']) {{
                    try {{
                        ...
                        if (barcodes.length == 1) {{
                            ...
                            if (barcode.rawValue === rawValue && barcode.rawValue !== confirmedValue) {{
                                confirmedValue = barcode.rawValue;
                        ...
                        console.log("BarcodeInput error", err);
...
#[component]
pub fn Barcode(barcode: Signal<String>) -> Element {
